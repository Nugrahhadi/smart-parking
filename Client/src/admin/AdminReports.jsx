...
const AdminReports = () => {
  ...
  const loadData = async () => {
    try {
      setLoading(true);
      const [chartData, metricsData, locationsData] = await Promise.all([
        fetchDashboardChart(),
        fetchDashboardMetrics(),
        fetchLocations(),
      ]);
      ...
      setLocations([{ id: "all", name: "All Locations" }, ...locationsData]);
      setError(null);
    } catch (err) {
  ...
  // Mock data for revenue chart (transform from usageData)
  const revenueData = usageData.map((item) => ({
    day: item.hour || item.day,
    revenue: (item.count || item.occupancy) * 50000,
  }));

  // Mock data for location distribution
  const locationData = locations.slice(1, 4).map((loc) => ({
    name: loc.name,
    value: Math.round(Math.random() * 50) + 20,
  }));

  // Mock data for hourly distribution
  const hourlyData = usageData.slice(0, 9).map((item, index) => ({
    hour: item.hour || `${6 + index * 2}:00`,
    value: item.count || item.occupancy || Math.round(Math.random() * 70) + 15,
  }));

  // Colors for charts
  const COLORS = ["#0088FE", "#00C49F", "#FFBB28", "#FF8042"];

  // Format currency - already imported from utils
    ...
    <div className="bg-white rounded-lg shadow-sm overflow-hidden">
      <div className="p-4 border-b">
        ...
        <p className="text-gray-600">
          View and export parking usage and revenue reports
        </p>
      </div>
      ...
      <div className="p-4 flex flex-wrap gap-3 border-b">
        ...
        <div className="flex space-x-2">
          <button
            className="px-3 py-2 border border-gray-300 rounded-md flex items-center text-gray-700"
            onClick={loadData}
          >
            ...
            <div className="grid grid-cols-4 gap-4 mb-6">
              <div className="bg-blue-50 p-4 rounded-lg">
                ...
                <p className="text-3xl font-bold text-blue-600">
                  {Math.round(metrics.avg_occupancy || 0)}%
                </p>
                <p className="text-sm text-blue-800 mt-1">Current average</p>
              </div>
              ...
              <div className="bg-green-50 p-4 rounded-lg">
                ...
                <p className="text-sm text-green-800 mt-1">Active now</p>
              </div>
              ...
              <div className="bg-purple-50 p-4 rounded-lg">
                ...
                <p className="text-sm text-purple-800 mt-1">This period</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-4 gap-4 mb-6">
              <div className="bg-green-50 p-4 rounded-lg">
                ...
                <p className="text-sm text-green-800 mt-1">
                  +{metrics.revenue_growth || 0}% from last period
                </p>
              </div>
              ...
              <div className="bg-blue-50 p-4 rounded-lg">
                ...
                <p className="text-sm text-blue-800 mt-1">Total count</p>
              </div>
              ...
              <div className="bg-purple-50 p-4 rounded-lg">
                ...
                <p className="text-sm text-purple-800 mt-1">Average amount</p>
              </div>
              ...
              <div className="bg-yellow-50 p-4 rounded-lg">
                ...
                <p className="text-sm text-yellow-800 mt-1">Peak day</p>
              </div>
            </div>
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {revenueData.map((day, index) => (
                      <tr key={index} className="hover:bg-gray-50">
                        ...
                        <td className="py-3 px-4">
                          {Math.floor(day.revenue / 19000)}
                        </td>
                        <td className="py-3 px-4">
                          Rp{" "}
                          {(
                            day.revenue / Math.floor(day.revenue / 19000)
                          ).toLocaleString()}
                        </td>
                        <td className="py-3 px-4">
                          {
                            locationData[
                              Math.floor(Math.random() * locationData.length)
                            ].name
                          }
                        </td>
                      </tr>
